...
`manymome.table` first. Because the
method is from the `flextable` package,
it also needs to be loaded:
...
The method `as_flextable()` can then be used
to convert the output to a flextable. Again, to use
this method, we need to load the package
`manymome.table` and `flextable`, if they are not
yet loaded:
